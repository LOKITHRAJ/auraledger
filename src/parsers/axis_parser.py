...
class AxisParser(BaseParser):
    ...
    def parse(self, file_path: Path) -> pd.DataFrame:
        logger.info(f"Parsing Axis statement: {file_path}")
        
        try:
            preview = pd.read_excel(file_path, header=None, dtype=str)
        except Exception as e:
            logger.error(f"Failed to read Axis file: {e}")
        ...
        if header_row is None:
            raise HeaderNotFoundException("Axis transaction header row not found.")

        logger.info(f"Detected Axis header row at index: {header_row}")
        ...
        column_mapping = {
            ...
            "reference": ["chq no", "chq", "ref", "reference"],
            "debit": ["withdrawals", "debit", "withdrawal"],
            "credit": ["deposits", "credit", "deposit"],
            "balance": ["balance", "bal"]
        }
        ...
        for std_key, patterns in column_mapping.items():
            ...
            if matched_col:
                ...
                raise HeaderNotFoundException(f"Required Axis column mapping not found for '{std_key}'")

...
class CUBParser(BaseParser):
    """
    Parser for City Union Bank (CUB) Excel statements.
    """

    def parse(self, file_path: Path) -> pd.DataFrame:
        logger.info(f"Parsing CUB statement: {file_path}")
        
        try:
            preview = pd.read_excel(file_path, header=None, dtype=str)
        except Exception as e:
            logger.error(f"Failed to read CUB file: {e}")
        ...
        if header_row is None:
            raise HeaderNotFoundException("CUB transaction header row not found.")

        logger.info(f"Detected CUB header row at index: {header_row}")
        ...
        column_mapping = {
            ...
            "reference": ["cheque no", "chq", "ref", "reference"],
        ...
        for std_key, patterns in column_mapping.items():
            ...
            if matched_col:
                ...
                raise HeaderNotFoundException(f"Required CUB column mapping not found for '{std_key}'")

        # Rename columns
        df = df.rename(columns={v: k for k, v in mapped_columns.items()})
        df = df[list(column_mapping.keys())]

        # Clean CUB header/footer (filter to valid dates)
